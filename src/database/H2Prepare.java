...
//        return stat;
//    }
    public static int insertRecords(String name, String path, String type, long size, long modified, long hash, String tags, long search_hash) {
        int stat = 0;
        try {
            try (Statement stmt = conn.createStatement()) {
                String sql = "INSERT INTO PUBLIC.FILES (NAME, PATH, EXTENTION, SIZE, MODIFIED, HASH, TAGS, SEARCH_HASH) "
                        + "VALUES ('" + name + "', '" + path + "', '" + type + "', " + size + ", " + modified + ", " + hash + ", '" + tags + "', " + search_hash + ")";
                stat = stmt.executeUpdate(sql);
                if (DEBUG) {
